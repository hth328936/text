...
def love():
    ...
    tk.Label(root, text="宝贝别生气啦！送你一个大爱心！", bg="black", fg="#FF99CC", font="Helvetic 12 bold").place(relx=.5, rely=.5,
                                                                                                anchor=CENTER)
    root.mainloop()
...
if __name__ == '__main__':
    ...
    tk.Label(root, text='表贝俺错啦', width=37, font=('宋体', 12)).place(x=0, y=10)
    ...
    def NO():  # 拒绝按钮，拒绝不会退出，必须同意才可以退出哦~
        tk.messagebox.showwarning('❤', '吴雪莲不要再生气啦')


    def closeWindow():
        tk.messagebox.showwarning('❤', '再给我一次机会嘛！')


    tk.Button(root, text='哼!勉为其难原谅你', width=14, height=1, command=OK).place(x=50, y=50)
    tk.Button(root, text='才不原谅你呢', width=10, height=1, command=OK).place(x=180, y=50)
